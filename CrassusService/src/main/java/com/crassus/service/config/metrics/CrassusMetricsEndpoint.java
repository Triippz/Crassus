...
/**
 * <p>CrassusMetricsEndpoint class.</p>
 */
@WebEndpoint(id = "crassusmetrics")
public class CrassusMetricsEndpoint {

  private final MeterRegistry meterRegistry;

  private final Logger logger = LoggerFactory.getLogger(CrassusMetricsEndpoint.class);
  ...
  /**
   * <p>Constructor for CrassusMetricsEndpoint.</p>
   *
   * @param meterRegistry a {@link MeterRegistry} object.
   */
  public CrassusMetricsEndpoint(MeterRegistry meterRegistry) {
    this.meterRegistry = meterRegistry;
  }

  /**
   * GET /management/ff-metrics
   * <p>
  ...
  private Map<String, Number> processMetrics() {
    ...
    Collection<TimeGauge> timeGauges = Search
      .in(meterRegistry)
  ...
  private Map<String, Object> garbageCollectorMetrics() {
    ...
    Collection<Timer> timers = Search
      .in(meterRegistry)
    ...
    gauges =
      Search.in(meterRegistry).name(s -> s.contains("jvm.classes.loaded")).gauges();
    ...
    Collection<FunctionCounter> functionCounters = Search
      ...
      .functionCounters();
    Double classesUnloaded = functionCounters
      .stream()
  ...
  private Map<String, Map<String, Number>> cacheMetrics() {
    ...
    Collection<FunctionCounter> counters = Search
      .in(meterRegistry)
  ...
  private Map<String, Map<String, Number>> jvmMemoryMetrics() {
    ...
    Search jvmMaxSearch = Search
      .in(meterRegistry)
      .name(s -> s.contains("jvm.memory.max"));
    ...
    gauges =
      Search.in(meterRegistry).name(s -> s.contains("jvm.memory.committed")).gauges();
    gauges.forEach(gauge -> {
      String key = gauge.getId().getTag("id");
