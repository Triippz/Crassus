package com.crassus.service.resources.errors;

import static org.springframework.core.annotation.AnnotatedElementUtils.findMergedAnnotation;

import com.crassus.core.CrassusConstants;
import com.crassus.errors.exceptions.BadRequestAlertException;
import com.crassus.errors.rest.ErrorConstants;
import com.crassus.errors.rest.FieldErrorVM;
import com.crassus.errors.rest.ProblemDetailWithCause;
import com.crassus.errors.utils.ErrorHeaderUtil;
...
public class ExceptionTranslator extends ResponseEntityExceptionHandler {
  ...
  private ProblemDetailWithCause getProblemDetailWithCause(Throwable ex) {
    if (
      ex instanceof
      ErrorResponseException exp &&
      exp.getBody() instanceof ProblemDetailWithCause problemDetailWithCause
    ) return problemDetailWithCause;
    return ProblemDetailWithCause.ProblemDetailWithCauseBuilder
      .instance()
  ...
  protected ProblemDetailWithCause customizeProblem(
    ProblemDetailWithCause problem,
    Throwable err,
    NativeWebRequest request
  ) {
    ...
    if (
      problemProperties == null || !problemProperties.containsKey(MESSAGE_KEY)
    ) problem.setProperty(
      MESSAGE_KEY,
    ...
    if (
      problemProperties == null || !problemProperties.containsKey(PATH_KEY)
  ...
  private HttpStatus toStatus(final Throwable throwable) {
    ...
    if (throwable instanceof ErrorResponse err) return HttpStatus.valueOf(
  ...
  private ResponseStatus resolveResponseStatus(final Throwable type) {
    final ResponseStatus candidate = findMergedAnnotation(
      type.getClass(),
  ...
  private String getCustomizedErrorDetails(Throwable err) {
    ...
    if (activeProfiles.contains(CrassusConstants.SPRING_PROFILE_PRODUCTION)) {
      if (
        err instanceof HttpMessageConversionException
  ...
  private HttpHeaders buildHeaders(Throwable err) {
    return err instanceof BadRequestAlertException badRequestAlertException
      ? ErrorHeaderUtil.createFailureAlert(
        applicationName,
        true,
