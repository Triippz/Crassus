package com.crassus.service.config.locale;
...
import com.crassus.service.test.LogbackRecorder;
import com.crassus.service.test.LogbackRecorder.Event;
...
class AngularCookieLocaleResolverTest {
  ...
  private static final TimeZone TIMEZONE_DEFAULT = TimeZone.getTimeZone(
    ZoneId.of("GMT+01:00")
  );
  ...
  void testDefaults() {
    ...
    assertThat(((TimeZoneAwareLocaleContext) context).getLocale())
      .isEqualTo(LOCALE_DEFAULT);
    assertThat(((TimeZoneAwareLocaleContext) context).getTimeZone())
      .isEqualTo(TIMEZONE_DEFAULT);
  ...
  void testPresets() {
    ...
    when(request.getAttribute(TIME_ZONE_REQUEST_ATTRIBUTE_NAME))
      .thenReturn(TIMEZONE_DEFAULT);
  ...
  void testLocaleAndTimeZoneWithQuotes() {
    String value = resolver.quote(
      LOCALE_CUSTOM.toString() + " " + TIMEZONE_CUSTOM.getID()
    );
    Cookie cookie = new Cookie(DEFAULT_COOKIE_NAME, value);
    when(request.getCookies()).thenReturn(new Cookie[] { cookie });
  ...
  void testTraceLogLocale() {
    ...
    assertThat(event.getMessage())
      .isEqualTo("Parsed cookie value [" + value + "] into locale '" + locale + "'");
